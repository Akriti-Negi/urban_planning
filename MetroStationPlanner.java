...
public class MetroStationPlanner {
    ...
    public double[][] floydWarshall(double[][] distanceMatrix) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                dist[i][j] = i == j ? 0 : 
                    (distanceMatrix[i][j] == 0 ? Double.POSITIVE_INFINITY : distanceMatrix[i][j]);
            }
        }
        ...
        shortestPaths = dist; // Save the result
        return dist;
    }

    // Method to get the stored shortest paths
    ...
    // Calculate Betweenness Centrality (placeholder method)
    public double[] calculateBetweennessCentrality(double[][] distanceMatrix) {
        int n = distanceMatrix.length;
        double[] centrality = new double[n];
        for (int i = 0; i < n; i++) {
            centrality[i] = Math.random(); // Dummy values
        }
        return centrality;
    }

    // Visualize the result
    public String visualizeStations(List<Map<String, Object>> data, int[] clusters) {
        StringBuilder visualization = new StringBuilder("Visualized Stations:\n");
        for (int i = 0; i < clusters.length; i++) {
            visualization.append("Station ").append(i).append(" -> Cluster ").append(clusters[i]).append("\n");
        }
        return visualization.toString();
    }

    // Getters for additional methods
